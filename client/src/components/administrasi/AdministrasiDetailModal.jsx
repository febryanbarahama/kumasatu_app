...
export default function AdministrasiDetailModal({
  open,
  data,
  onClose,
  baseURL,
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div className="w-full max-w-3xl p-6 bg-white rounded-lg shadow-lg dark:bg-gray-800">
          ...
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold">Detail Permohonan Layanan</h3>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <Info label="Jenis Layanan" value={data.jenis_layanan} />
            <Info label="No HP" value={data.no_hp} />

            <div className="md:col-span-2">
              <p className="text-sm text-gray-500">Keperluan</p>
              <p className="whitespace-pre-line">{data.keperluan}</p>
            </div>
          </div>
          ...
          <div className="mt-5">
            ...
            <div className="flex flex-col gap-3">
              {data.lampiran_ktp && (
                <Attachment label="KTP" path={data.lampiran_ktp} />
              )}

              {data.lampiran_kk && (
                <Attachment label="KK" path={data.lampiran_kk} />
              )}

              {data.lampiran_lainnya && (
                <Attachment
                  label="Lampiran Lainnya"
                  path={data.lampiran_lainnya}
                />
              )}
